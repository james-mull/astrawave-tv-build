...
class TmdbSeriesEpisodeRepository(
    ...
    fun load(seriesId: Long): List<StremioEpisode> = loadSeries(seriesId).episodes

    fun loadSeries(seriesId: Long): SeriesEpisodes {
        if (seriesId <= 0L || apiBaseUrl.isBlank()) return SeriesEpisodes(seriesId, null, emptyList())
        ...
        return SeriesEpisodes(
            seriesId = seriesId,
            title = root.optString("title").takeIf { it.isNotBlank() && it != "null" },
            episodes = parsed,
        )
    }
}
