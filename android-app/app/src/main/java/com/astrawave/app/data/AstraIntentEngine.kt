...
object AstraIntentEngine {
    fun parse(query: String): AstraIntent {
        val raw = query.trim()
        val q = raw.lowercase()
        val runtime = Regex("(?:under|less than|within)\\s+(\\d{2,3})\\s*(?:min|minutes)")
            .find(q)?.groupValues?.getOrNull(1)?.toIntOrNull()
            ?: Regex("(?:under|less than)\\s+(\\d(?:\\.\\d+)?)\\s*hours?")
                .find(q)?.groupValues?.getOrNull(1)?.toDoubleOrNull()?.let { (it * 60).toInt() }
        val rating = Regex("(?:rated|rating|over|above)\\s+(\\d(?:\\.\\d+)?)")
            .find(q)?.groupValues?.getOrNull(1)?.toDoubleOrNull()
        val similar = Regex("(?:like|similar to)\\s+(.+)$", RegexOption.IGNORE_CASE)
            .find(raw)?.groupValues?.getOrNull(1)?.trim()
        val team = Regex("(?:put|play|watch|show)\\s+(?:the\\s+)?(.+?)\\s+(?:game|match)", RegexOption.IGNORE_CASE)
            .find(raw)?.groupValues?.getOrNull(1)?.trim()
        val type = when {
