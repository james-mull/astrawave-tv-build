...
class SportsPreferenceStore(context: Context) {
    private val prefs=context.getSharedPreferences("astrawave_sports_preferences_v1",Context.MODE_PRIVATE)
    ...
    fun saveReminder(profileId:String,item:Reminder){val all=reminders(profileId).filterNot{it.id==item.id}+item;val a=JSONArray();all.forEach{a.put(JSONObject().put("id",it.id).put("eventId",it.eventId).put("title",it.title).put("startTime",it.startTime).put("enabled",it.enabled))};prefs.edit().putString("reminders_$profileId",a.toString()).apply()}
    fun removeReminder(profileId:String,id:String){val all=reminders(profileId).filterNot{it.id==id};val a=JSONArray();all.forEach{a.put(JSONObject().put("id",it.id).put("eventId",it.eventId).put("title",it.title).put("startTime",it.startTime).put("enabled",it.enabled))};prefs.edit().putString("reminders_$profileId",a.toString()).apply()}
}
