            ...
            for (i in 0 until items.length()) {
                ...
                add(
                    CloudEvent(
                        ...
                        homeScore = item.optInt("homeScore").takeIf { item.has("homeScore") && !item.isNull("homeScore") },
                        awayScore = item.optInt("awayScore").takeIf { item.has("awayScore") && !item.isNull("awayScore") },
                        statusDetail = item.optString("statusDetail").takeIf(String::isNotBlank),
