...
/** Premium 2/3/4/6-pane Media3 multiview with one audible pane and sports mosaic mode. */
@Composable
fun MultiviewScreen(
    ...
    var activeAudioPaneId by remember(session.id) { mutableStateOf(session.activeAudioPaneId ?: session.panes.firstOrNull()?.id) }
    ...
    Column(Modifier.fillMaxSize().background(AstraWaveColors.Background).padding(24.dp)) {
        Text(if (session.layout == MultiviewLayout.SPORTS_MOSAIC) "Sports Mosaic" else "Multiview", color = AstraWaveColors.PrimaryText, style = MaterialTheme.typography.headlineLarge)
        Spacer(Modifier.height(6.dp))
        Text(
            if (session.layout == MultiviewLayout.SPORTS_MOSAIC) "Event-first six-pane viewing. Select any pane to move audio instantly; open a pane for full-screen playback."
            else "Watch up to six live channels or sports events at once where device hardware permits. Only the selected pane plays audio.",
        ...
        when (session.layout) {
            ...
            MultiviewLayout.FOUR_UP -> Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                repeat(2) { row -> Row(Modifier.weight(1f), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    PaneOrEmpty(session,row*2,Modifier.weight(1f),activeAudioPaneId,::activate,onOpenPane,onReplacePane)
                    PaneOrEmpty(session,row*2+1,Modifier.weight(1f),activeAudioPaneId,::activate,onOpenPane,onReplacePane)
                } }
            }
            MultiviewLayout.SIX_UP, MultiviewLayout.SPORTS_MOSAIC -> Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                repeat(2) { row -> Row(Modifier.weight(1f), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    repeat(3) { col -> PaneOrEmpty(session,row*3+col,Modifier.weight(1f),activeAudioPaneId,::activate,onOpenPane,onReplacePane) }
                } }
            }
        }
    }
...
private fun MultiviewPlayerSurface(pane:MultiviewPane,audible:Boolean,modifier:Modifier=Modifier){
    ...
    val player=remember(pane.id,pane.streamUrl){ExoPlayer.Builder(context).build().apply{repeatMode=androidx.media3.common.Player.REPEAT_MODE_OFF;setMediaItem(MediaItem.fromUri(pane.streamUrl));volume=if(audible)1f else 0f;prepare();playWhenReady=true}}
    LaunchedEffect(audible,player){player.volume=if(audible)1f else 0f}
    DisposableEffect(player){onDispose{player.release()}}
    AndroidView(modifier=modifier.background(AstraWaveColors.Background),factory={PlayerView(it).apply{useController=false;this.player=player}},update={it.player=player})
}
