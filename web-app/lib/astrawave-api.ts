...
export type SportsChannelMatch = { id:string; name:string; logoUrl?:string; group?:string; matchScore:number; matchReason?:string; sourceCount:number; healthScore?:number; candidates:LiveStreamSource[] };
export type SportsEvent = { id:string; league:string; sport?:string; title:string; startTime:string; status:string; broadcaster?:string; broadcasts?:string[]; homeTeam?:string; awayTeam?:string; badge?:string; date?:string; source?:string; channelMatches?:SportsChannelMatch[]; watchable?:boolean };
export type SportsChannelCloud = { generatedAt:string; dates:string[]; stats:{events:number;eventsWithBroadcasts:number;eventsWithChannelMatches:number;channels:number;healthyChannels:number}; events:SportsEvent[]; channels?:Array<{id:string;name:string;group?:string;logoUrl?:string;sourceCount:number;healthScore?:number;online?:boolean;sources:LiveStreamSource[]}>; failures?:string[]; policy?:string };
...
async function titleSearch(query:string){const server=await getJson<CatalogItem[]>(`/v1/search?q=${encodeURIComponent(query)}`);if(server.length||!browserTmdbToken())return server;const data=await browserTmdb(`/search/multi?query=${encodeURIComponent(query)}&include_adult=false&language=en-US`);return(data.results||[]).filter((x:any)=>x.media_type==='movie'||x.media_type==='tv').slice(0,30).map((x:any)=>mapTmdb([x],x.media_type==='movie'?'movie':'series')[0])}
async function search(query:string){
  const q=query.trim().toLowerCase();if(!q)return[];
  const [titlesResult,liveResult,sportsResult,audioResult]=await Promise.allSettled([
    titleSearch(query),
    getJson<LiveData>('/live-data?source=all-free'),
    getJson<SportsChannelCloud>('/sports-channel-cloud?days=3'),
    fetch('https://de1.api.radio-browser.info/json/stations/topvote/80?hidebroken=true',{cache:'no-store'}).then(async r=>{if(!r.ok)throw new Error(`Radio directory ${r.status}`);return r.json() as Promise<any[]>}),
  ]);
  const titles=titlesResult.status==='fulfilled'?titlesResult.value:[];
  const live=liveResult.status==='fulfilled'?liveResult.value.channels.filter(c=>{
    const hay=[c.name,c.group,c.now,c.next,...(c.programs||[]).map(p=>p.title)].filter(Boolean).join(' ').toLowerCase();return hay.includes(q)
  }).slice(0,24).map(c=>({id:c.id,kind:'live' as const,title:c.name,subtitle:[c.now||c.group,`${c.sourceCount} source${c.sourceCount===1?'':'s'}`].filter(Boolean).join(' • '),posterUrl:c.logoUrl,streamUrl:c.streamUrl,sources:c.sources})):[];
  const sports=sportsResult.status==='fulfilled'?sportsResult.value.events.filter(e=>[e.title,e.league,e.sport,...(e.broadcasts||[]),e.homeTeam,e.awayTeam,e.status].filter(Boolean).join(' ').toLowerCase().includes(q)).slice(0,20).map(e=>{const match=e.channelMatches?.[0];const sources=match?.candidates||[];return{id:e.id,kind:'sport' as const,title:e.title,subtitle:[e.league,e.status,(e.broadcasts||[]).join(' / '),e.startTime].filter(Boolean).join(' • '),posterUrl:e.badge,streamUrl:sources[0]?.streamUrl,sources}}):[];
  const audio=audioResult.status==='fulfilled'?audioResult.value.filter(s=>[s.name,s.country,s.tags].filter(Boolean).join(' ').toLowerCase().includes(q)).slice(0,16).map(s=>({id:String(s.stationuuid||s.changeuuid||s.name),kind:'podcast' as const,title:String(s.name||'Public radio'),subtitle:[s.country,s.tags?.split(',')?.[0]].filter(Boolean).join(' • '),posterUrl:s.favicon||undefined,streamUrl:s.url_resolved||s.url})):[];
  const seen=new Set<string>();return[...titles,...live,...sports,...audio].filter(item=>{const key=`${item.kind}:${item.id}`;if(seen.has(key))return false;seen.add(key);return true}).slice(0,70)
}
async function titleDetails(kind:'movie'|'series',id:string){const server=await getJson<{configured:boolean;details?:TitleDetails}>(`/title/${kind}/${encodeURIComponent(id)}`);if(server.configured||!browserTmdbToken())return server;const isMovie=kind==='movie';const data=await browserTmdb(isMovie?`/movie/${id}?language=en-US&append_to_response=credits,videos,recommendations,similar,release_dates`:`/tv/${id}?language=en-US&append_to_response=credits,videos,recommendations,similar,content_ratings,external_ids`);let rating:string|null=null;if(isMovie){const us=data.release_dates?.results?.find((x:any)=>x.iso_3166_1==='US');rating=(us?.release_dates||[]).map((x:any)=>String(x.certification||'').trim()).find(Boolean)||null}else rating=data.content_ratings?.results?.find((x:any)=>x.iso_3166_1==='US')?.rating||null;const trailers=(data.videos?.results||[]).filter((v:any)=>v.site==='YouTube'&&['Trailer','Teaser','Clip','Featurette'].includes(v.type)).slice(0,8).map((v:any)=>({key:v.key,name:v.name,type:v.type,official:Boolean(v.official),url:`https://www.youtube.com/watch?v=${encodeURIComponent(v.key)}`}));const related=mapTmdb([...(data.recommendations?.results||[]),...(data.similar?.results||[])],kind).filter(x=>x.id!==id).slice(0,24);return{configured:true,details:{id,kind,title:data.title||data.name||'Untitled',overview:data.overview||'',tagline:data.tagline||'',posterUrl:tmdbImage(data.poster_path),backdropUrl:tmdbImage(data.backdrop_path,'original'),releaseDate:data.release_date||data.first_air_date||undefined,runtimeMinutes:isMovie?Number(data.runtime||0)||null:Number(data.episode_run_time?.[0]||0)||null,rating,score:Number(data.vote_average||0),voteCount:Number(data.vote_count||0),genres:(data.genres||[]).map((g:any)=>g.name),status:data.status||undefined,seasons:Number(data.number_of_seasons||0)||undefined,episodes:Number(data.number_of_episodes||0)||undefined,homepage:data.homepage||undefined,imdbId:String(isMovie?data.imdb_id||'':data.external_ids?.imdb_id||'').trim()||undefined,cast:(data.credits?.cast||[]).slice(0,12).map((p:any)=>({id:String(p.id),name:p.name,role:p.character||undefined,profileUrl:tmdbImage(p.profile_path,'w185')})),crew:(data.credits?.crew||[]).filter((p:any)=>['Director','Creator','Writer','Screenplay','Executive Producer'].includes(p.job)).slice(0,10).map((p:any)=>({id:String(p.id),name:p.name,role:p.job})),trailers,related}}}

function localStremioManifests(){
  try{
    if(typeof window==='undefined')return[] as {name?:string;url:string}[];
...
export const AstraWaveApi={
  home:()=>getJson<{rows:CatalogRail[]}>('/v1/home'),catalogRails,addonRails:()=>getJson<{reviewedOnly:boolean;manifests:string[];addons:any[];rails:CatalogRail[]}>('/addon-rails'),sourceRegistry:()=>getJson<SourceRegistry>('/source-registry'),liveData:(source='all-free')=>getJson<LiveData>(`/live-data?source=${encodeURIComponent(source)}`),sportsChannelCloud:(date?:string,days=3)=>getJson<SportsChannelCloud>(`/sports-channel-cloud?days=${Math.min(Math.max(days,1),7)}${date?`&date=${encodeURIComponent(date)}`:''}`),sportsData:()=>getJson<SportsChannelCloud>('/sports-channel-cloud?days=3'),titleDetails,
  trendingMovies:async()=>{const r=await catalogRails('movie');return r.rails[0]?.items||[]},trendingShows:async()=>{const r=await catalogRails('series');return r.rails[0]?.items||[]},sources:unifiedSources,liveChannels:async()=>(await getJson<LiveData>('/live-data?source=all-free')).channels,guide:()=>getJson<LiveData>('/live-data?source=all-free'),sportsToday:async()=>(await getJson<SportsChannelCloud>('/sports-channel-cloud?days=1')).events,
