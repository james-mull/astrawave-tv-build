...
const mdblistPaths:Record<string,string>={
  'movie:popular-movies':'official/movies/popular',
  'movie:most-watched-movies':'official/movies/most-watched',
  'movie:most-anticipated-movies':'official/movies/anticipated',
  'show:popular-shows':'official/shows/popular',
  'show:most-watched-shows':'official/shows/most-watched',
  'show:most-anticipated-shows':'official/shows/anticipated',
};
...
function mapMdb(payload:any,kind:'movie'|'series'){
  ...
  return (source||[]).flatMap((x:any)=>{
    const tmdb=String(x.tmdbid||x.id||'').trim();const imdb=String(x.imdbid||'').trim();const id=tmdb||imdb;const title=String(x.title||x.name||'').trim();if(!id||!title)return[];
    const key=`${kind}:${id}`;if(seen.has(key))return[];seen.add(key);
    return[{id,kind,title,subtitle:String(x.release_year||x.year||x.released||'').trim()||undefined,posterUrl:x.poster||x.poster_url||undefined,overview:x.description||undefined,score:Number(x.score||0)||undefined,externalIds:{tmdb:tmdb||undefined,imdb:imdb||undefined,trakt:x.traktid||undefined}}];
  }).slice(0,60);
}
...
export async function GET(request:NextRequest){
  ...
  if(!id){const definitions=kind==='series'?tvCatalogs:movieCatalogs;return NextResponse.json({kind,count:definitions.length,definitions,mdblistConfigured:Boolean(process.env.MDBLIST_API_KEY)},{headers:{'Cache-Control':'public, s-maxage=14400, stale-while-revalidate=86400'}});}
  ...
  const listPath=mdblistPaths[id];
  if(listPath&&process.env.MDBLIST_API_KEY){
    try{const payload=await mdblist(listPath,definition.kind);const items=mapMdb(payload,definition.kind);if(items.length)return NextResponse.json({configured:true,definition,source:'MDBList',items}, {headers:cacheHeaders});}catch{/* fall through to TMDB */}
  }
  ...
  try{const path=definition.tmdbPath||fallbackSearchPath(definition.title,definition.kind);const payload=await tmdb(path);const items=mapTmdb(payload?.results||[],definition.kind);return NextResponse.json({configured:true,definition,source:definition.tmdbPath?'TMDB dynamic catalog':'TMDB metadata fallback',items},{headers:cacheHeaders});}
  catch(error){return NextResponse.json({configured:false,definition,items:[],error:error instanceof Error?error.message:'Catalog unavailable'},{status:200,headers:cacheHeaders});}
}
