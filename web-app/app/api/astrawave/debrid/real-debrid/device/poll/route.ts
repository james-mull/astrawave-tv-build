...
export async function POST(request:NextRequest){
  try{
    ...
    return NextResponse.json({connected:true,accessToken:token.access_token,refreshToken:token.refresh_token||null,expiresIn:Number(token.expires_in||0),tokenType:token.token_type||'Bearer'});
