...
const fallbackRows: { title: string; items: CatalogItem[] }[] = [
  { title:'Welcome to AstraWave', items:[
    {id:'1',kind:'movie',title:'Connect TMDB for live catalog'},
    {id:'2',kind:'live',title:'Add authorized Live TV feed'},
    {id:'3',kind:'sport',title:'Sports schedules are ready'},
    {id:'4',kind:'podcast',title:'Music & podcasts coming online'},
  ]}
];
...
export default function WebAppHome(){
  ...
  useEffect(()=>{
    AstraWaveApi.home()
      .then((data)=>{ if(data.rows.length) setRows(data.rows); else setNotice('Connect TMDB and AstraWave feeds to populate your live catalog.'); })
      .catch(()=>setNotice('AstraWave is ready. Add provider credentials to activate live catalog data.'))
  ...
  return <main className="appShell">
    ...
    <section className="appMain"><header className="appTop"><div><small>ASTRAWAVE WEB</small><h1>What are you watching?</h1></div><div className="topActions"><button className="searchBox"><Search size={17}/> Search AstraWave</button><button className="round"><Library size={19}/></button></div></header>
      ...
      {notice && <section className="contentRow"><div className="rowHead"><h3>{notice}</h3></div></section>}
