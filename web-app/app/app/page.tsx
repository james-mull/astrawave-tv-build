...
export default function WebAppHome(){
  ...
  return <main className="appShell">
    ...
    <section className="appMain"><header className="appTop"><div><small>ASTRAWAVE WEB</small><h1>What are you watching?</h1></div><div className="topActions"><button className="searchBox"><Search size={17}/> Search AstraWave</button><button className="round"><Library size={19}/></button></div></header>
      ...
      {playing?.streamUrl && <section className="contentRow playerPanel"><div className="rowHead"><h3>Now Playing: {playing.title}</h3><button onClick={()=>setPlaying(null)}>Close</button></div><video src={playing.streamUrl} controls playsInline autoPlay />{playing.sources?.length ? <div className="sourceChips">{playing.sources.map((source,index)=><button className={source.streamUrl===playing.streamUrl?'active':''} key={source.id} onClick={()=>setPlaying({...playing,streamUrl:source.streamUrl})}>Source {index+1}<span>{source.provider}</span></button>)}</div> : null}</section>}
