...
export default function WebAppHome(){
  ...
  return <main className="shell">
    ...
    <section className="main"><header className="top"><button className="menu" onClick={()=>setMobileNav(true)}><Menu/></button><div><small>ASTRAWAVE</small><h1>{pageTitle}</h1></div><div className="search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search movies & TV"/></div><button className="user"><UserCircle2/></button></header>
      ...
      {!query&&view==='live'&&<><Intro k="LIVE TV" h="Change sources instantly." p="Use one merged lineup or isolate AstraWave Free TV, IPTV Nexus US, public TV, IPTV.org Sports and other reviewed lineups."/><SourceTabs options={liveData.sourceOptions} active={activeSource} onSelect={switchSource}/><div className="live-tools"><div><b>{liveData.stats.channels}</b><span>channels</span></div><div><b>{liveData.stats.epgLinked}</b><span>EPG linked</span></div><input value={liveQuery} onChange={e=>setLiveQuery(e.target.value)} placeholder="Search channels, groups, programs"/></div><div className="channel-grid">{filteredLive.slice(0,liveLimit).map(c=><button key={c.id} onClick={()=>c.streamUrl&&play({id:c.id,kind:'live',title:c.name,subtitle:c.now||c.group,posterUrl:c.logoUrl,streamUrl:c.streamUrl,sources:c.sources})}><div className="logo">{c.logoUrl?<img src={c.logoUrl} alt=""/>:<RadioTower/>}</div><div><b>{c.name}</b><small>{c.now||c.group||'Live programming'}</small><span>{c.sourceCount} source{c.sourceCount===1?'':'s'}{c.next?` • Next: ${c.next}`:''}</span></div><Play size={14}/></button>)}</div>{filteredLive.length>liveLimit&&<button className="secondary" onClick={()=>setLiveLimit(v=>Math.min(v+160,filteredLive.length))}>Show more channels • {filteredLive.length-liveLimit} remaining</button>}</>}

      {!query&&view==='guide'&&<><Intro k="EPG" h="The guide is back." p={`${liveData.stats.epgLinked} live channels are currently matched to AstraWave’s cached EPG; the EPG cache reports ${liveData.stats.epgScheduledChannels||0} scheduled channels.`}/><SourceTabs options={liveData.sourceOptions} active={activeSource} onSelect={switchSource}/><div className="guide"><header><span>Channel</span><span>Now</span><span>Next</span><span>Following</span></header>{guideChannels.slice(0,guideLimit).map(c=><button key={c.id} onClick={()=>c.streamUrl&&play({id:c.id,kind:'live',title:c.name,streamUrl:c.streamUrl,sources:c.sources})}><b>{c.name}</b><span>{c.now||'—'}</span><span>{c.next||'—'}</span><span>{c.programs?.[2]?.title||'—'}</span></button>)}</div>{guideChannels.length>guideLimit&&<button className="secondary" onClick={()=>setGuideLimit(v=>Math.min(v+120,guideChannels.length))}>Show more guide channels • {guideChannels.length-guideLimit} remaining</button>}</>}
