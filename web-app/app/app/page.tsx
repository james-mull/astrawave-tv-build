...
export default function WebAppHome(){
  ...
  return <main className="shell">
    ...
    {!query.trim()&&view==='sports'&&<section><div className="dateChips"><button className={sportsDate==='all'?'active':''} onClick={()=>setSportsDate('all')}>All</button>{dates.slice(0,14).map(d=><button key={d} className={sportsDate===d?'active':''} onClick={()=>setSportsDate(d)}>{new Date(`${d}T12:00:00`).toLocaleDateString([],{month:'short',day:'numeric'})}</button>)}</div><div className="sportsGrid">{visibleSports.map(e=>{const c=matchedChannel(e);return <article key={e.id}><span>{e.league}</span><h3>{e.title}</h3><p>{new Date(e.startTime).toLocaleString()} • {e.status}</p><small>{e.broadcaster||'Broadcaster TBD'}</small>{c&&<button className="primaryBtn" onClick={()=>play({id:c.id,kind:'live',title:c.name,subtitle:e.title,posterUrl:c.logoUrl,streamUrl:c.streamUrl,sources:c.sources})}>Watch on {c.name}</button>}</article>})}</div></section>}
