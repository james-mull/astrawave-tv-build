...
function sourceScore(source:SourceCandidate){
  ...
  const release=/remux|bluray/i.test(source.releaseType||'')?4:/web-dl/i.test(source.releaseType||'')?2:0;
  const audio=/atmos|truehd|dts-hd/i.test(source.audioCodec||'')?4:/dts|e-ac-3|ac-3/i.test(source.audioCodec||'')?2:0;
  return resolution+direct+bitrate+uptime+latency+format+release+audio;
}
...
function SmartVodPlayer({kind,id,title,cloudUid,sources,selectedIndex,onSelectedIndex,onEnded}:{kind:string;id:string;title:string;cloudUid:string|null;sources:SourceCandidate[];selectedIndex:number;onSelectedIndex:(index:number)=>void;onEnded?:()=>void}){
  ...
  const playerMeta=[selected.quality,selected.releaseType,selected.codec,selected.hdr,selected.audioCodec,selected.language,formatFileSize(selected.fileSizeBytes),selected.licenseLabel].filter(Boolean);
  return <div className="player"><video ref={videoRef} key={selected.url} src={selected.url} controls playsInline autoPlay/><div className="player-meta"><ShieldCheck size={15}/><span><b>{selected.provider}</b>{playerMeta.length?` • ${playerMeta.join(' • ')}`:''}</span></div>{message&&<p className="player-message">{message}</p>}</div>
}
...
export default function TitlePlaybackPage({ params }: { params: Promise<{ kind: string; id: string }> }) {
  ...
  return <main className="title-shell">
    ...
    {!loading&&details&&<>
      ...
      <section className="section" id="player"><div className="section-head"><div><small>SMART PLAYBACK</small><h2>{selectedEpisode?`Play Best • S${selectedEpisode.season} E${selectedEpisode.episode}`:'Play Best'}</h2></div><span>{sources.length} approved candidate{sources.length===1?'':'s'}</span></div>{sources.length===0?<div className="state"><ShieldCheck/> {selectedEpisode?episodeMessage||'No approved episode source is currently available.':'No approved playable source is currently available. Add permitted Stremio, debrid or provider sources in the Control Center.'}</div>:<><div className="smart-summary"><div><b>{selectedEpisode?selectedEpisode.title:selected?.provider}</b><span>{selectedEpisode?`S${selectedEpisode.season} E${selectedEpisode.episode} • ${selected?.provider||'Best source'}`:`${selected?.quality||'Quality not reported'}${selected?.releaseType?` • ${selected.releaseType}`:''}${selected?.audioCodec?` • ${selected.audioCodec}`:''}${selected?.direct?' • Direct':''}`}</span></div><p>AstraWave ranks eligible sources by resolution, direct-play capability, bitrate, uptime, latency, efficient video formats, release quality and premium audio metadata when those signals are available. Larger files are shown for context but are not automatically ranked higher.{selectedEpisode?` AutoNext is ${autoNext?'enabled':'disabled'} for this account.`:''}{uid?' Progress is synced to your signed-in AstraWave account.':''}</p></div><div className="source-list">{sources.map((source,index)=>{const size=formatFileSize(source.fileSizeBytes);const metadata=[source.quality,source.releaseType,source.codec,source.hdr,source.audioCodec,source.language,size,source.bitrateKbps?`${Math.round(source.bitrateKbps/1000)} Mbps`:null,source.uptimePercent!=null?`${source.uptimePercent.toFixed(1)}% uptime`:null,source.latencyMs!=null?`${Math.round(source.latencyMs)} ms`:null,source.direct?'Direct':null,source.licenseLabel].filter(Boolean);return <button key={source.id} className={selectedIndex===index?'active':''} onClick={()=>setSelectedIndex(index)}><div><b>{index+1}. {source.provider}</b><small>{metadata.join(' • ')||'Eligible source'}</small></div><span>{selectedIndex===index?'Playing':`Score ${Math.round(sourceScore(source))}`}</span></button>})}</div>{route&&<SmartVodPlayer kind={playbackKind} id={playbackId} title={playbackTitle} cloudUid={uid} sources={sources} selectedIndex={selectedIndex} onSelectedIndex={setSelectedIndex} onEnded={selectedEpisode&&autoNext?playNextEpisode:undefined}/>}</>}</section>
