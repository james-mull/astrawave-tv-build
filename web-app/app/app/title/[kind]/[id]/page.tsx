...
import { ArrowLeft, Bookmark, Check, ChevronRight, ExternalLink, Film, Play, ShieldCheck, Star, Tv2 } from 'lucide-react';
...
function SmartVodPlayer({kind,id,sources,selectedIndex,onSelectedIndex}:{kind:string;id:string;sources:SourceCandidate[];selectedIndex:number;onSelectedIndex:(index:number)=>void}){
  ...
  useEffect(()=>{
    ...
    video.addEventListener('timeupdate',save);video.addEventListener('pause',save);video.addEventListener('ended',()=>{try{localStorage.removeItem(playbackKey(kind,id))}catch{}});video.addEventListener('error',fail);
    return()=>{save();video.removeEventListener('timeupdate',save);video.removeEventListener('pause',save);video.removeEventListener('error',fail)};
  },[kind,id,selected?.url,selectedIndex,sources.length,onSelectedIndex]);

  if(!selected?.url)return null;
  return <div className="player"><video ref={videoRef} key={selected.url} src={selected.url} controls playsInline autoPlay/><div className="player-meta"><ShieldCheck size={15}/><span><b>{selected.provider}</b>{selected.quality?` • ${selected.quality}`:''}{selected.codec?` • ${selected.codec}`:''}{selected.licenseLabel?` • ${selected.licenseLabel}`:''}</span></div>{message&&<p className="player-message">{message}</p>}</div>
}
