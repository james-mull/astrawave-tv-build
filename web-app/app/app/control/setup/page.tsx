...
export default function EasySetupPage(){
  ...
  const requiresValidation=kind==='stremio'||kind==='cloudstream';
  ...
  async function save(){
    ...
    if(requiresValidation&&!probeOk){setMessage(`Check this ${kind==='stremio'?'manifest':'repository'} first. AstraWave only saves structurally validated public addon definitions.`);return}
    ...
    try{
      ...
      setMessage(kind==='cloudstream'?'✓ Repository definition validated and synced. Community plugins remain non-executable until AstraWave policy explicitly allows them.':'✓ Added and synced. Your signed-in AstraWave devices can use this configuration.');setProbeOk(true)
  ...
  return <main className="easySetup"><header><small>ASTRAWAVE EASY CONTENT SETUP</small><h1>Add content without the technical mess.</h1><p>Choose how you already access content, paste the one URL you were given, and AstraWave handles the configuration. Playback still requires sources you are authorized to use.</p></header>
    ...
    <section className="wizard"><div className="wizardHead"><SelectedIcon/><div><small>STEP 1 OF 2</small><h2>{selected[1]}</h2><p>{selected[2]}</p></div></div>
      ...
      <div className="actions">{canProbe&&<button className="secondary" onClick={probe} disabled={probing}><ShieldCheck/> {probing?'Checking…':'Check source'}</button>}<button className="primary" onClick={save} disabled={saving||probing}><CheckCircle2/> {saving?'Saving…':requiresValidation&&!probeOk?'Validate, then add':'Add to AstraWave'}</button></div>{message&&<div className={probeOk?'notice ok':'notice'}>{message}</div>}
    </section>
    <section className="how"><article><Link2/><h3>Stremio</h3><p>Paste a manifest URL. AstraWave validates the manifest structure before saving it, then syncs the definition across devices for eligible authorized resources.</p></article><article><Database/><h3>CloudStream</h3><p>Repository structure is validated before saving. Enabled repos are discovery/configuration only; community plugin execution stays isolated and separately policy-gated.</p></article><article><RadioTower/><h3>IPTV</h3><p>M3U + XMLTV and Xtream are available for customer-authorized providers. The player can still use Unified and Provider Order behavior.</p></article><article><Server/><h3>Your media</h3><p>Plex and Jellyfin give AstraWave another clean path to content you already own or are authorized to access.</p></article></section>
    <div className="policy"><ShieldCheck/> AstraWave organizes and plays eligible sources. Validation confirms structure—not content rights. It does not turn an untrusted repository or addon into an authorized source automatically.</div>
