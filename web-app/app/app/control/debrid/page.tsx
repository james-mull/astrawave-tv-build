...
export default function DebridPage(){
  ...
  async function verify(){
    const response=await fetch('/api/astrawave/debrid/real-debrid/verify',{method:'POST',headers:{'content-type':'application/json'},body:'{}',cache:'no-store'});
  ...
  async function connect(){
    ...
    try{
      ...
      while(!cancelRef.current&&Date.now()<deadline){
        ...
        const result=await poll.json() as {connected?:boolean;error?:string};
        if(!poll.ok||!result.connected)throw new Error(result.error||'Authorization failed');
        await verify();setDevice(null);setMessage('✓ Real-Debrid connected for this browser session. Play Best can now optimize already-authorized sources.');return;
      }
  ...
  async function disconnect(){
    cancelRef.current=true;setConnecting(false);
    try{await fetch('/api/astrawave/debrid/real-debrid/disconnect',{method:'POST',cache:'no-store'})}catch{}
    setProfile(null);setDevice(null);setMessage('Real-Debrid disconnected from this browser session.');
  }
