...
export default function DebridPage(){
  ...
  async function verify(token?:string){
    const response=await fetch('/api/astrawave/debrid/real-debrid/verify',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(token?{accessToken:token}:{}),cache:'no-store'});
  ...
  useEffect(()=>{
    ...
    verify(token).catch(()=>{sessionStorage.removeItem(TOKEN_KEY);setProfile(null)});
  ...
  async function disconnect(){
    ...
    sessionStorage.removeItem(TOKEN_KEY);setProfile(null);setDevice(null);setMessage('Real-Debrid disconnected from this browser session.');
  }
